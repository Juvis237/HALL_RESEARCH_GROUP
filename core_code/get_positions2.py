...
#combined_set = [f'{num:05}' for num in list(range(0, 251, 10)) + list(range(252, 523, 2))]
#prefix = 'mwc' # edit this based on the Run number

#nop = 1 #number of planets gives number of rows
 #len(combined_set) gives number of column


 

def track_planet_position (nop, prefix, combined_set, current_directory = os.getcwd()):
    y=0 
    ...
    df2 = pd.DataFrame(angles, columns=column_labels, index=row_labels)


    # Save DataFrame to a CSV file
    df.to_csv(f'{prefix}_positions.csv', index=False)
    df2.to_csv(f'{prefix}_angular_position.csv', index=False)
